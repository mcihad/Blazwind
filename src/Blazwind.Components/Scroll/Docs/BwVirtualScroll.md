...
A high-performance component for rendering large lists by only injecting elements that are currently visible within the
viewport. Ideal for datasets with thousands of items.
...
> [!IMPORTANT]
> `ItemHeight` must be a fixed value for the virtualization logic to calculate offsets correctly. Variable height items
