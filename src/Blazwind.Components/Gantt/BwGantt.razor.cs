...
public partial class BwGantt : BwBase, IAsyncDisposable
{
    ...
    private bool _isResizingTaskList;
    ...
    [Parameter]
    public GanttViewMode ViewMode { get; set; } = GanttViewMode.Week;
    ...
    [Parameter]
    public EventCallback<GanttTask> OnTaskClick { get; set; }

    [Parameter]
    public EventCallback<GanttTaskResizeEventArgs> OnTaskResize { get; set; }

    [Parameter]
    public EventCallback<GanttTaskDragEventArgs> OnTaskDrag { get; set; }
    ...
    [Parameter]
    public int TaskListWidth { get; set; } = 256;

    /// <summary>Allow resizing the task list panel</summary>
    [Parameter]
    public bool TaskListResizable { get; set; } = true;

    /// <summary>Custom template for task list items</summary>
    [Parameter]
    public RenderFragment<GanttTask>? TaskItemTemplate { get; set; }

    /// <summary>Custom context menu template for task bars</summary>
    [Parameter]
    public RenderFragment<GanttTask>? ContextMenuTemplate { get; set; }
    ...
    [Parameter]
    public bool ShowTodayIndicator { get; set; } = true;

    // Task List Events
    /// <summary>Fired when task list visibility changes</summary>
    [Parameter]
    public EventCallback<bool> OnTaskListVisibilityChanged { get; set; }

    /// <summary>Fired when task list width changes after resize</summary>
    [Parameter]
    public EventCallback<int> OnTaskListWidthChanged { get; set; }
    ...
    private double CalculateTodayOffset()
    {
        ...
        for (var i = 0; i < PeriodBoundaries.Count; i++)
        {
    ...
    private List<GanttTask> FlattenTasks(List<GanttTask> tasks, int level = 0)
    {
        ...
        {
            result.Add(task);
            if (task.Children.Count > 0) result.AddRange(FlattenTasks(task.Children, level + 1));
        }
    ...
    private string GetTaskBarStyle(GanttTask task, int rowIndex)
    {
        ...
        if (ViewMode == GanttViewMode.Month && PeriodBoundaries.Count > 0)
        {
            ...
            for (var i = 0; i < PeriodBoundaries.Count && remainingDuration > 0; i++)
            {
                var periodStart = PeriodBoundaries[i];
                var periodEnd = i < PeriodBoundaries.Count - 1 ? PeriodBoundaries[i + 1] : _timelineEnd.AddDays(1);
                var periodDays = (periodEnd - periodStart).TotalDays;

                if (currentDate >= periodEnd) continue;
        ...
        return
            $"left: {startOffset.ToString("F2", CultureInfo.InvariantCulture)}px; width: {width.ToString("F2", CultureInfo.InvariantCulture)}px; top: {top}px; height: 20px;";
    }
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        else if (!Editable && _isJsInitialized) await DisposeJsAsync();
    ...
    public async Task HandleTaskResizeFromJs(string taskId, int startDaysDelta, int endDaysDelta)
    {
        ...
        // Update task dates
        if (startDaysDelta != 0) task.StartDate = task.StartDate.AddDays(startDaysDelta);
        if (endDaysDelta != 0) task.EndDate = task.EndDate.AddDays(endDaysDelta);
    ...
    {
        var task = FlatTasks.FirstOrDefault(t => t.Id == taskId);
        if (task != null) await OnTaskClick.InvokeAsync(task);
    }
    ...
    private RenderFragment RenderTaskBar(GanttTask task)
    {
        return builder =>
        {
            if (task.IsMilestone)
            ...
            else
            {
                ...
                if (task.Progress > 0)
                {
                    ...
                    builder.AddAttribute(10, "style",
                        $"width: {task.Progress}%; background-color: {task.Color ?? "var(--bw-color-primary)"};");
                    builder.CloseElement();
                }
                ...
                if (ShowBarLabels)
                {
                    ...
                    if (task.Progress > 0 && task.Progress < 100) builder.AddContent(16, $" ({task.Progress}%)");
                }
                ...
                builder.AddAttribute(21, "class",
                    "absolute left-0 bottom-full mb-1 hidden group-hover:block z-30 pointer-events-none");
                builder.OpenElement(22, "div");
                builder.AddAttribute(23, "class",
                    "bg-gray-800 dark:bg-gray-900 text-white text-[10px] rounded px-2 py-1 whitespace-nowrap shadow-lg");
